...
class FakeStructuredLlm:
    def __init__(self, response: RouterOutput):
...
def test_router_output_rejects_unknown_route():
    with pytest.raises(ValidationError):
        RouterOutput(route="reuse", route_reason="지원하지 않는 분기입니다.")
...
def test_build_chain_input_formats_documents_and_copies_history(policy_document):
    ...
    chain_input = router._build_chain_input(
...
def test_decide_uses_structured_output_and_renders_router_context(policy_document):
    ...
    assert llm.schema is RouterOutput
...
def test_decide_forces_retriever_without_invoking_llm_when_documents_are_empty():
    llm = FakeStructuredLlm(
        RouterOutput(
            route="agent",
            route_reason="이 응답은 호출되면 안 됩니다.",
        )
    )
    router = PolicyRouter(llm)
    ...
    assert result == RouterOutput(
        route="retriever",
        route_reason="documents가 비어있습니다: 강제 Retrieval 진행",
    )
    assert llm.prompt_values == []
...
def test_adecide_forces_retriever_without_invoking_llm_when_documents_are_empty():
    llm = FakeStructuredLlm(
        RouterOutput(
            route="agent",
            route_reason="이 응답은 호출되면 안 됩니다.",
